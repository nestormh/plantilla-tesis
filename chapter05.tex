...
Now we know the relative movement of each voxel, we can proceed to segment the obstacles based on this information. This process comprises three steps: segmentation, in which an initial segmentation of the obstacles is performed; filtering, in order to remove the remaining noise; and aggregation, which tries to join obstacles that were initially split, but are actually parts of a single obstacle.

\subsection{Segmentation}\label{ch:chapter05_01_05_01}
...
\For {\textbf{each} voxel $g \in \mathcal{G'}$}
  ...
  \While {$queue \ne \emptyset$}
    ...
    \EndFor
  \EndWhile
  \If {$|o| >= \tau_{min\_voxels\_per\_obstacle}}$}
...
The neighborhood of a voxel $g$ represented by the function $N(g)$, as in \cite{broggi2013}, is performed using the Chebyshev distance. When a new voxel $g''$ is added to an obstacle $o$, the voxel identifier is updated. From the obstacle point of view, we just add the voxel to a list associated to each obstacle. In the future, this list will be used for updating the information related to the obstacle centroid, speed and limits on each dimension. This process could be online, saving computational resources, but as we are performing the filtering and aggregation steps, we have to wait until those steps will be finished.

\subsection{Filtering}\label{ch:chapter05_01_05_02}

\subsection{Aggregation}\label{ch:chapter05_01_05_03}

\begin{figure}[th]
  \centering
  \includegraphics{obstacleSpeed}
  \caption{\todo{Explicar esto cuando ya haya terminado con la fase de segmentacion. Rehacer la imagen}.}\label{fig:cp05_obstacle_speed}
\end{figure}

\subsection{Ego-motion compensation}\label{ch:chapter05_01_05_04}
