...
% TODO: Decir que una de las cosas que se asumen es que no va a haber un gran cambio
% En el caso test0,1,22,23 seguimos con el mismo problema, pero 28b nos soluciona la papeleta????
We also wanted to know which of the methods was more tolerant to low frame rate sequences, as one of the assumptions for the tracking methods is that the temporal difference between frames is not too big. Based on this, we evaluated the relation existing between the recall, $\Delta$ frames and the time step between frames. So we repeated the tests, but this time increasing $k$ frames each time, with $k=0.06\dots1.2\,s$. Then, we evaluated the variation in the results, obtaining the values represented in figure \ref{fig:cp04_recall_vs_delta_frames_vs_step}.

% TODO: Mostrar la parte deltatime-recall para todos y luego el 3d con una seleccion
\begin{figure}[h!]
\centering
...
% AlfaHeight no aporta nada
% Es mejor usar 2 level pq da mejores resultados ¿aunque si el frame rate puede llegar a compensr el obstacle?
% El obstacle es el mas rapido, puede interesar si los recursos son limitados o se prefiere velocidad frente a eficiencia
% 
% Trabajo futuro: Utilizar el polar calibration para la generación de los propios stixels?
Usar el propio tracking como medida a tener en cuenta durante el proceso de clustering
